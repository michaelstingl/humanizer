...
- **Abstrakte Containerwörter streichen.** "Aspekt", "Bereich", "Rahmen", "Thematik", "Problematik", "Faktor", "Komponente" stehen oft an der Stelle, an der die konkrete Sache stehen müsste. "im Bereich der Datenhaltung" → "in der Datenhaltung"; "verschiedene Aspekte der Konfiguration" → benennen, welche. Die Wörter sind nicht verboten, aber wo sie sich streichen lassen, ohne dass Information verlorengeht, waren sie Polsterung. Abgrenzung: in Fachtexten sind "Faktor" und "Komponente" häufig Termini mit fester Bedeutung und bleiben dann stehen. *([KT★] Muster 13 Abstraktionswörter; konvergent mit [HDE] "Abstrakta". Zwei unabhängige deutschsprachige Quellen, kein EN-Transfer.)*
...
- **Kein Signposting an den Leser.** "Schauen wir uns an", "werfen wir einen Blick auf", "im Folgenden betrachten wir". Der Abschnittstitel leistet das bereits. *([HDE]; deutschsprachige Entsprechung zu §28 Signposting and Announcements, unabhängig belegt.)*
...
- **Keine Aphorismen als Absatzschluss.** Der abrundende Merksatz ("Qualität entsteht nicht zufällig.") ist eine Modellgewohnheit, kein Argument. *([HDE]; deckt die Absatzschluss-Position ab und überschneidet sich dort mit §32 Aphorism Formulas. Die von §32 zusätzlich erfassten Formelkonstruktionen ("X ist die Sprache des Y", "X wird zur Falle") sind hier noch nicht geprüft — siehe Open questions.)*
...
- [WP-DE] [Wikipedia:Anzeichen für KI-generierte Inhalte](https://de.wikipedia.org/wiki/Wikipedia:Anzeichen_f%C3%BCr_KI-generierte_Inhalte). German counterpart to en:Wikipedia:Signs of AI writing. Import September 2025, actively maintained. Primary source for German AI-tell patterns. **Stand dieses Presets: oldid 268532846 (5. Juli 2026).** Änderungen seither: `https://de.wikipedia.org/w/index.php?title=Wikipedia:Anzeichen_für_KI-generierte_Inhalte&diff=cur&oldid=268532846`. Beim Nachziehen die oldid hier mit aktualisieren, sonst zeigt der Diff wieder auf einen überholten Stand.
- [HDE] [marmbiz/humanizer-de](https://github.com/marmbiz/humanizer-de). German AI Text Humanizer für Claude Code und Codex, Version 5.19.0 (Stand 14. August 2026). 72 deutsche Muster in 10 Kategorien, davon ca. 15 als deterministische Python-Linter (`humanizer_audit.py`, `evidence_lint.py`, `rhythm_lint.py`), optionale spaCy-Syntaxanalyse, Evidence-Gate zum Schutz von Zahlen, Namen und Belegen. Kein GitHub-Fork von blader/humanizer, sondern eigenständiges Repo, das Teile daraus ableitet (siehe dessen NOTICE); Musterkatalog aus [WP-DE] abgeleitet, nicht aus dem Englischen übersetzt. MIT, Musterkatalog-Material jedoch CC BY-SA 4.0. Sehr aktiv gewartet (tägliche Releases, ~25 Test-Suiten, CI). Ausführlichste bekannte DE-Quelle.
...
- [KT] / [KT★] [severinschweiger/klartext](https://github.com/severinschweiger/klartext). Claude skill specifically for German AI-pattern removal. 36 patterns, 7 marked deutschspezifisch (★). April 2026; seither unverändert, praktisch eingeschlafen (alle Commits an einem Tag). Alle sieben ★-Muster sind hier abgedeckt (Stand 14. August 2026): Muster 9 "jedoch"/"aber", 10 Genitivketten, 11 archaische Demonstrative, 12 "sowohl … als auch", 13 Abstraktionswörter, 14 aufzählende Absatzanfänge, 15 übertriebene Formalität. The ★ marker in this document points at klartext's German-only rules. Quelle der Quick-Check-Idee.
- [TX] [LangeVC/txtHumanizer](https://github.com/LangeVC/txtHumanizer). Drei-Stufen-Workflow für deutschsprachige Texte (ANALYSE → RECOMMEND → FINETUNE). Evidenzbasiert auf Wikipedia KI-Erkennung v1.35. Apache 2.0. Letzter Stand Juli 2026. Vorsicht bei der Beispieltransformation im README: sie erfindet Kennzahlen und Firmenangaben, was gegen die No-Fabrication-Regel (SKILL.md) verstößt. Als Vorbild für Beispiele nicht geeignet.
...
- Wikipedia:DE has been "import-and-adapt" from EN since September 2025. Has the DE community diverged in a direction the EN page does not cover? The diff link in the [WP-DE] source entry answers the first half cheaply now that the base revision is recorded; the comparison against the EN page (oldid 1369390317, 14 August 2026) still has to be read by hand.
...
- Upstream v2.8.0 added §31 (manufactured punchlines and staccato drama), §32 (aphorism formulas) and §33 (conversational rhetorical openers). Current coverage differs per pattern and none has formally been through the transferability check:
  - **§32 is partly covered already.** "Keine Aphorismen als Absatzschluss" reaches the same tell in closing position, from a German source ([HDE]), arrived at independently. What is missing is §32's wider family of formula constructions anywhere in a paragraph ("X ist die Sprache des Y", "X wird zur Falle"). Extend the existing rule rather than adding a second one, and keep the German citation.
  - **§31 is not covered.** The Rhythmus section addresses the opposite failure, uniform sentence length, and explicitly permits long German sentences. A run of short declaratives staged for drama is a separate tell. Likely transferable (the mechanism is a cadence borrowed from ad copy, not an English surface form), but it needs German exemplars and a citation.
  - **§33 is not covered and is the doubtful one.** German technical prose has its own opener repertoire; "Honestly?" and "Here's the thing" have no direct equivalents, and the nearest German candidates ("Ganz ehrlich:", "Sagen wir es so:") belong to a more casual register than this preset targets. Check before assuming a tell exists here at all.
  - Note that §33 is *not* the counterpart of "Kein Signposting an den Leser". That rule corresponds to §28 Signposting and Announcements, which predates v2.8.0. The two are separate tells and were briefly conflated during the v2.9.1 sync.
